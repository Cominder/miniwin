...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            current_aux_loss = outputs.aux_loss.item() if lm_config.use_moe else 0.0
            current_lr = optimizer.param_groups[-1]['lr']
            eta_min = spend_time / max(step - start_step, 1) * (iters - step) // 60
            Logger(
                f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}), loss: {current_loss:.4f}, dpo: {current_dpo_loss:.4f}, '
                f'aux: {current_aux_loss:.4f}, lr: {current_lr:.8f}, epoch_time: {eta_min:.3f}min'
            )
            if wandb:
                wandb.log({
                    "loss": current_loss,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniWin DPO (Direct Preference Optimization)")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniWin-DPO", help="wandb项目名")
    ...
    if args.use_wandb and is_main_process():
        wandb_run_name = f"MiniWin-DPO-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb = init_wandb_run(args.wandb_project, wandb_run_name, ckp_data)
    
    # ========== 5. 定义模型和参考模型 ==========
    model, tokenizer = init_model(lm_config, args.from_weight, device=args.device)
    Logger(f'MiniWin策略模型总参数量：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    # 初始化参考模型（ref_model冻结）
    ref_model, _ = init_model(lm_config, args.from_weight, device=args.device)
    ref_model.eval()
    ref_model.requires_grad_(False)
    Logger(f'MiniWin参考模型总参数量：{sum(p.numel() for p in ref_model.parameters()) / 1e6:.3f} M')
