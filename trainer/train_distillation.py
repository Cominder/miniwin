    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            Logger(
                ...
                f'aux_loss: {current_aux_loss:.4f}, distill: {distill_loss.item():.4f}, lr: {current_lr:.8f}, epoch_time: {eta_min:.3f}min'
            )
            if wandb:
                wandb.log({
                    "loss": current_loss,
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lm_checkpoint(
                lm_config_student, weight=args.save_weight, model=model, optimizer=optimizer, scaler=scaler,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniWin Knowledge Distillation（对齐 minimind3）")
    ...
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="学生/教师是否使用 MoE")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniWin-Distillation", help="wandb项目名")
    parser.add_argument("--student_n_windows", type=int, default=2)
    parser.add_argument("--teacher_n_windows", type=int, default=2)
    parser.add_argument("--student_sliding_window_size", type=int, default=64)
    parser.add_argument("--teacher_sliding_window_size", type=int, default=64)
    parser.add_argument("--student_attention_pattern", type=str, default="SSSL")
    parser.add_argument("--teacher_attention_pattern", type=str, default="SSSL")
    parser.add_argument("--student_attention_pattern_force_last_global", type=int, default=1, choices=[0, 1])
    parser.add_argument("--teacher_attention_pattern_force_last_global", type=int, default=1, choices=[0, 1])
    parser.add_argument("--student_partial_rotary_factor", type=float, default=1.0)
    parser.add_argument("--teacher_partial_rotary_factor", type=float, default=1.0)
    parser.add_argument("--use_compile", default=0, type=int, choices=[0, 1], help="学生模型 torch.compile")
    ...
    lm_config_student = MiniWinConfig(
        hidden_size=args.student_hidden_size,
        num_hidden_layers=args.student_num_layers,
        use_moe=bool(args.use_moe),
        n_windows=args.student_n_windows,
        sliding_window_size=args.student_sliding_window_size,
        attention_pattern=args.student_attention_pattern,
        attention_pattern_force_last_global=bool(args.student_attention_pattern_force_last_global),
        partial_rotary_factor=args.student_partial_rotary_factor,
    )
    lm_config_teacher = MiniWinConfig(
        hidden_size=args.teacher_hidden_size,
        num_hidden_layers=args.teacher_num_layers,
        use_moe=bool(args.use_moe),
        n_windows=args.teacher_n_windows,
        sliding_window_size=args.teacher_sliding_window_size,
        attention_pattern=args.teacher_attention_pattern,
        attention_pattern_force_last_global=bool(args.teacher_attention_pattern_force_last_global),
        partial_rotary_factor=args.teacher_partial_rotary_factor,
    )
    ...
    if args.use_wandb and is_main_process():
        wandb_run_name = f"MiniWin-Distill-S{args.student_hidden_size}T{args.teacher_hidden_size}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
    ...
    Logger(f'MiniWin学生模型总参数量：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    teacher_model, _ = init_model(lm_config_teacher, args.from_teacher_weight, device=args.device)
    teacher_model.eval()
    teacher_model.requires_grad_(False)
    Logger(f'MiniWin教师模型总参数量：{sum(p.numel() for p in teacher_model.parameters()) / 1e6:.3f} M')
    ...
    if args.use_compile == 1:
        ...
        Logger('torch.compile enabled (student)')
    if dist.is_initialized():
