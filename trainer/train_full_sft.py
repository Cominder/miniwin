...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniWin Full SFT（对齐 minimind3 训练流程）")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniWin-Full-SFT", help="wandb项目名")
    ...
    parser.add_argument("--attention_pattern_force_last_global", type=int, default=1, choices=[0, 1], help="最后一层强制全局注意力 L")
    parser.add_argument("--partial_rotary_factor", type=float, default=1.0, help="仅前 rope_dim 维做 RoPE，其余为 nope。factor=1.0 表示全头 RoPE。")
    parser.add_argument("--use_compile", default=0, type=int, choices=[0, 1], help="是否使用 torch.compile（0=否，1=是）")
    ...
    if device_type == "cpu":
        ...
        try:
            ...
            if args.dtype == "bfloat16" and "bfloat16" in str(e).lower():
                if not dist.is_initialized() or is_main_process():
                    Logger("当前 GPU / PyTorch 不支持 bfloat16 autocast，已自动改用 float16")
                args.dtype = "float16"
                dtype = torch.float16
                autocast_ctx = torch.cuda.amp.autocast(dtype=dtype)
            else:
    ...
    if args.use_wandb and is_main_process():
        wandb_run_name = (
            f"MiniWin-Full-SFT-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
