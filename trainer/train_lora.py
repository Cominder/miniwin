...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            current_logits_loss = current_loss - current_aux
            current_lr = optimizer.param_groups[-1]['lr']
            eta_min = spend_time / max(step - start_step, 1) * (iters - step) // 60
            Logger(
                f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}) loss:{current_loss:.4f} '
                f'logits_loss:{current_logits_loss:.4f} aux:{current_aux:.4f} lr:{current_lr:.8f} epoch_time:{eta_min:.1f}min'
            )
            if wandb:
                wandb.log({"loss": current_loss, "logits_loss": current_logits_loss, "aux_loss": current_aux, "lr": current_lr, "epoch_time": eta_min})
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lm_checkpoint(
                lm_config, weight=args.lora_name, model=model, optimizer=optimizer, scaler=scaler,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniWin LoRA Fine-tuning")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniWin-LoRA", help="wandb项目名")
    ...
    parser.add_argument("--use_compile", default=0, type=int, choices=[0, 1], help="torch.compile（LoRA 训练慎用）")
    ...
    lm_config = MiniWinConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers, use_moe=bool(args.use_moe),
    ...
    if args.use_wandb and is_main_process():
        wandb_run_name = f"MiniWin-LoRA-{args.lora_name}-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}-SlidingPeType-{args.sliding_pe_type}-SlidingWindowSize-{args.sliding_window_size}-AttentionPattern-{args.attention_pattern}-AttentionPatternForceLastGlobal-{args.attention_pattern_force_last_global}"
