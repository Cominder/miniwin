...
def init_model(args):
    ...
    tokenizer = PreTrainedTokenizerFast.from_pretrained(args.load_from, trust_remote_code=True)
    if 'model' in args.load_from:
        model = MiniWinForCausalLM(MiniWinConfig(
    ...
    print(f'MiniWin 模型参数: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)')
    return model.eval().to(args.device), tokenizer

def main():
    parser = argparse.ArgumentParser(description="MiniWin 模型推理与对话")
    ...
    parser.add_argument('--max_new_tokens', default=8768, type=int, help="最大生成长度（注意：并非模型实际长文本能力）")
    parser.add_argument('--temperature', default=0.85, type=float, help="生成温度，控制随机性（0-1，越大越随机）")
    parser.add_argument('--top_p', default=0.85, type=float, help="nucleus采样阈值（0-1）")
    parser.add_argument('--open_thinking', default=0, type=int, choices=[0, 1], help="chat_template 是否开启 open_thinking（与 Reason 权重可叠加）")
    parser.add_argument('--show_speed', default=1, type=int, choices=[0, 1], help="是否打印 decode 速度 tokens/s")
    ...
    parser.add_argument("--partial_rotary_factor",type=float,default=1.0,help="部分 RoPE：仅 head 前 partial_rotary_factor 比例维度做 RoPE，1.0 与旧版全 RoPE 一致")
    ...
    prompts = [
        ...
        '解释什么是机器学习',
        '推荐一些中国的美食',
        '请写一篇关于人工智能发展历史的长文，从图灵测试到深度学习，再到大语言模型，详细介绍每个阶段的代表性技术、关键人物和里程碑事件。',
    ]
    ...
    for prompt in prompt_iter:
        ...
        if args.weight != 'pretrain':
            ...
            if args.weight == 'reason' or args.open_thinking:
                tmpl_kw["open_thinking"] = True
            try:
                text_in = tokenizer.apply_chat_template(conversation, **tmpl_kw)
            except TypeError:
                tmpl_kw.pop("open_thinking", None)
                text_in = tokenizer.apply_chat_template(conversation, **tmpl_kw)
        else:
            text_in = tokenizer.bos_token + prompt
        ...
        generated_ids = model.generate(
            ...
            top_p=args.top_p, temperature=args.temperature, repetition_penalty=1.1,
        )
        n_new = generated_ids.shape[1] - inputs["input_ids"].shape[1]
        if args.show_speed and n_new > 0:
            dt = time.perf_counter() - t0
            print(f"\n[decode ~{n_new / dt:.1f} tokens/s | {n_new} tokens]", end='')
